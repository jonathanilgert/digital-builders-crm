...
export default function WorkingHours() {
  ...
  const logActive = draftHours && parseFloat(draftHours) > 0;

  return (
    <div style={{ display: 'flex', flexDirection: 'column', flex: 1, minHeight: 0 }}>
      <div style={{ padding: pad }}>
        <h1 style={{ margin: 0, fontSize: isMobile ? 19 : 22, fontWeight: 600, letterSpacing: '-0.02em', color: T.ink }}>Working Hours</h1>
      </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            <Card padding={0}>
              <div style={{ padding: '12px 14px' }}>
                ...
                <div style={{ fontSize: 24, fontWeight: 600, color: T.ink, letterSpacing: '-0.02em', marginTop: 5, fontVariantNumeric: 'tabular-nums' }}>
                  {todayHours.toFixed(1)}<span style={{ fontSize: 13, color: T.mute, fontWeight: 500, marginLeft: 3 }}>hrs</span>
                </div>
              </div>
            </Card>
            <Card padding={0}>
              <div style={{ padding: '12px 14px' }}>
                ...
                <div style={{ fontSize: 24, fontWeight: 600, color: T.ink, letterSpacing: '-0.02em', marginTop: 5, fontVariantNumeric: 'tabular-nums' }}>
                  {weekTotal.toFixed(1)}<span style={{ fontSize: 13, color: T.mute, fontWeight: 500, marginLeft: 3 }}>hrs</span>
                </div>
              </div>
            </Card>
          </div>
          ...
          <Card padding={0}>
            ...
            <div style={{
              ...
              <button onClick={log} disabled={!logActive} style={{
                padding: '8px 14px', borderRadius: 8,
                height: isMobile ? 40 : 36,
                width: isMobile ? '100%' : undefined,
                whiteSpace: 'nowrap',
                background: logActive ? T.blue : T.bg,
                border: `1px solid ${logActive ? T.blue : T.line}`,
                color: logActive ? '#fff' : T.faint,
                fontSize: 12.5, fontWeight: 600,
                cursor: logActive ? 'pointer' : 'not-allowed',
              }}>Log hours</button>
            </div>
          </Card>
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                ...
                <button onClick={goThis} style={{
                  ...
                  background: T.card, border: `1px solid ${T.line}`,
                  fontSize: 11, fontWeight: 500, color: T.mid, cursor: 'pointer',
                }}>This week</button>
                ...
                {!isMobile && (
                  <span style={{
                    fontSize: 11.5, color: T.mid, fontWeight: 500, marginLeft: 4,
                    fontVariantNumeric: 'tabular-nums', minWidth: 110, textAlign: 'right',
                  }}>{weekLabel}</span>
                )}
              </div>
              {isMobile && (
                <span style={{ fontSize: 11, color: T.mute, fontWeight: 500, width: '100%' }}>{weekLabel}</span>
              )}
            </div>

            <div style={{ padding: isMobile ? '6px 14px 16px' : '8px 20px 20px' }}>
              <div style={{
                display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)',
                gap: isMobile ? 6 : 12, alignItems: 'end',
                ...
                {weekDays.map((d, i) => {
                  ...
                  return (
                    <div key={i} style={{
                      ...
                      <span style={{
                        fontSize: isMobile ? 9 : 10.5, fontWeight: 600,
                        fontVariantNumeric: 'tabular-nums',
                        color: v > 0 ? T.ink2 : T.faint,
                        opacity: v > 0 ? 1 : 0,
                      }}>{v.toFixed(1)}</span>
                      <div style={{
                        width: '100%', maxWidth: isMobile ? 28 : 36,
                        height: `${Math.max(pct, v > 0 ? 4 : 0)}%`,
                        minHeight: v > 0 ? 4 : 0,
                        background: isToday ? T.blue : T.blueBar,
                        borderRadius: 4, transition: 'height .25s ease',
                      }} />
                    </div>
                  );
                })}
              ...
              <div style={{
                display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)',
                gap: isMobile ? 6 : 12, marginTop: 6,
                borderTop: `1px solid ${T.lineSoft}`, paddingTop: 7,
              }}>
                {weekDays.map((d, i) => {
                  const isToday = isSameDay(d, today);
                  return (
                    <div key={i} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 1 }}>
                      <span style={{ fontSize: isMobile ? 9.5 : 11, fontWeight: isToday ? 600 : 500, color: isToday ? T.blue : T.mid }}>
                        {DAY_NAMES[i].slice(0, isMobile ? 1 : 3)}
                      </span>
                      <span style={{ fontSize: isMobile ? 9 : 10, color: T.mute, fontVariantNumeric: 'tabular-nums' }}>
                        {d.getDate()}
                      </span>
                    </div>
                  );
